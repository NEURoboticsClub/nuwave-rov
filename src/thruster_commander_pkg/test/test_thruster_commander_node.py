...
def commander_node(executor):
    ...
    from thruster_commander.new_thruster_commander_node import ThrusterCommander
...
class TestNodeStartup:
    def test_node_creates_publishers(self, commander_node):
        """Commander should create one publisher per thruster."""
        assert len(commander_node.thruster_pubs) == commander_node.n_thrusters
    ...
    def test_zero_twist_gives_neutral_pwm(self, executor, commander_node, publisher, listener):
        ...
        for i, val in latest.items():
            ...
            assert abs(val - commander_node.neutral_us) < 1.0, \
                f"Thruster {i}: expected ~{commander_node.neutral_us}, got {val}"
...
class TestMessageFlow:
    def test_timer_publishes_without_input(self, executor, commander_node, listener):
        ...
        assert received_count == commander_node.n_thrusters, \
            f"Only {received_count}/{commander_node.n_thrusters} channels received"
